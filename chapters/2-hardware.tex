...
多处理器架构（multiprocessing architecture）\index{多处理器架构multiprocessing architecture}\index[en]{multiprocessing architecture多处理器架构}是指一个计算机系统拥有多个物理的处理器，或者拥有多个核（core）\myindex{核}{core}，两者的主要区别为多核处理器每个核仅拥有的L1缓存及寄存器，同一芯片内的核共享L2及主内存，而多个单独的物理处理器往往仅共享主存并拥有自己的缓存系统。
